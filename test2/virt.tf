resource "libvirt_network" "monsoon_test" {
  ...
  dns {
    enabled = true
    hosts {
      hostname = "gateway.k8s2.local"
      ip       = "192.168.101.1"
    }
    hosts {
      hostname = "controller102.k8s2.local"
      ip       = "192.168.101.68"
    }
    hosts {
      hostname = "worker102.k8s2.local"
      ip       = "192.168.101.158"
    }
  }
  ...
  dnsmasq_options {
    ...
    options {
      ...
      option_value = "tag:efi-x64-pxe,\"http://gateway.k8s2.local:8080/assets/ipxe/x86_64/undionly.kpxe\""
    }
    ...
    options {
      option_name  = "dhcp-boot"
      option_value = "tag:efi-x64-http,\"http://gateway.k8s2.local:8080/assets/ipxe/x86_64/undionly.kpxe\""
    }
    options {
    ...
    options {
      option_name  = "dhcp-boot"
      option_value = "tag:efi-aa64-http,\"http://gateway.k8s2.local:8080/assets/ipxe/aarch64/snponly.efi\""
    }
    options {
      option_name  = "dhcp-option-force"
