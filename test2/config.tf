  ...
  snippets_worker100 = concat(
    ...
    [
      templatefile(
        ...
        {
          MACAddress = local.workers.worker102.mac,
      }),
      templatefile(
        ...
        {
          diskno    = 2
          used_disk = local.enable_install ? local.workers.worker102.install_disk : local.workers.worker102.persist_disk
      }),
    ]
  )
